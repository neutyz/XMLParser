...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  return [[[theClass alloc] initConsumingXMLNode:theXMLNode] autorelease]; //tianyz
}
...
+ (id)nodeBorrowingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  return [[[theClass alloc] initBorrowingXMLNode:theXMLNode] autorelease]; //tianyz
}
    ...
    if (buff) {
      ...
      if (result > -1) {
        str = [[[NSString alloc] initWithBytes:(xmlBufferContent(buff))
                                      ...
                                      encoding:NSUTF8StringEncoding] autorelease]; //tianyz
      }
      xmlBufferFree(buff);
    }
  }
